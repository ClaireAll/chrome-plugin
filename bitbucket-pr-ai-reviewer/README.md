...
审查内容包括 PR 标题、PR 描述、提交信息、变更文件列表和展开上下文后的非测试文件 diff 分块。扩展会跳过 `test.ts`、`*.test.ts`、`*.spec.ts` 以及 `test`、`tests`、`__tests__` 目录下的测试文件改动，不会把这些文件作为 AI finding 的目标。扩展会先让 DeepSeek 推断本次变更目的，再重点检查逻辑问题、行为回归、边界情况、API 契约不一致、权限变化和缺失测试。
