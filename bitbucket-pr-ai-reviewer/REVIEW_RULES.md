...
- 不审查测试文件自身的 diff，例如 `test.ts`、`*.test.ts`、`*.spec.ts`，以及 `test`、`tests`、`__tests__` 目录下的文件；也不审查 `.md` 文档文件改动。不要为这些文件输出 finding。
- 审查 `.json` 文件时只围绕本次增删的 key/value 判断，不要根据相邻未改动 JSON key 或缺少完整上下文推断问题。
- 减少“可能导致”“可能存在”这类猜测型结论；当前 diff 能支撑明确风险路径时要输出 finding，例如条件分支、数据流、调用链、接口/组件契约、状态流转、删除符号、残留引用或渲染结果冲突。
- 每条问题必须能拆出结构化证据：改动代码证据、触发条件、调用链/数据流/状态流/组件契约，以及已经检查过但未推翻该结论的上下文证据；只有缺少改动代码依据或已被上下文反证时才不输出。
- 清晰输出：每个 diff 分块最多返回 5 条 finding，`detail` 按“哪处改动 -> 什么场景 -> 会出什么错”的顺序用一句话说明，`suggestion` 用一句话说清楚具体改法，证据字段只写短句。
- 文案按给 PR 作者留言的方式写：保留 diff/context 支撑的事实，不为了顺口补充新影响；避免“建议确认”“需要注意”“存在一定风险”这类 AI 套话；不要在 title、detail、suggestion 里重复同一句依据。
...
证据要求：
1. 减少“可能导致”“可能存在”“建议确认”这类猜测型 review；当前 diff、PR 标题/描述、commit message、组件源码或已有上下文能支撑明确风险路径时要输出。
2. 每条问题的 detail 必须说明触发条件和逻辑依据，例如哪段改动改变了条件分支、数据结构、调用链、接口契约、状态流转或渲染结果，以及为什么会产生实际错误。
3. 如果缺少改动代码依据、不了解项目约定且 diff/context 也无法支撑风险路径，或已检查上下文反证该问题，不要输出 finding；可以保持空 findings。
4. 不审查测试文件自身的 diff，例如 `test.ts`、`*.test.ts`、`*.spec.ts`，以及 `test`、`tests`、`__tests__` 目录下的文件；也不审查 `.md` 文档文件改动。不要为这些文件输出 finding。
5. 对 `.json` 文件，只审查本次增删的 key/value，不要因为相邻未改动 key 看起来相关就输出 finding。
6. 清晰输出：每个 diff 分块最多 5 条 finding，detail 按“哪处改动 -> 什么场景 -> 会出什么错”的顺序写一句话，suggestion 一句话说清楚具体改法，证据字段用短句。
7. 文案按给 PR 作者留言的方式写：保留 diff/context 支撑的事实，不为了顺口补充新影响；避免“建议确认”“需要注意”“存在一定风险”这类 AI 套话；不要在 title、detail、suggestion 里重复同一句依据。
...
2. 可以判断业务改动是否缺少测试覆盖，但不要审查测试文件自身的实现改动，也不要审查 `.md` 文档文件改动。
