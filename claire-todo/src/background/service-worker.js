...
if (globalThis.chrome?.runtime?.onStartup) {
  chrome.runtime.onStartup.addListener(() => {
    pauseRunningTimersOnStartup().catch(() => {});
...
async function handleMessageNow(message = {}, sender) {
  ...
  switch (message.type) {
    ...
    case MESSAGE_TYPES.TOGGLE_TODO_TIMER:
      return toggleTimer(payload);
    case MESSAGE_TYPES.TOGGLE_FOCUSED_TODO_TIMER:
      return toggleFocusedTimer(payload);
    case MESSAGE_TYPES.TOUCH_TODO_TIMER:
      return touchTimer(payload);
...
// 切换指定待办的计时状态，并持久化最新列表。
async function toggleTimer(payload) {
  return saveItems(toggleTodoTimer(await loadTodoItems(), payload.id, payload.now));
}

// 切换悬浮球当前关注的计时任务。
async function toggleFocusedTimer(payload) {
  return saveItems(toggleFocusedTodoTimer(await loadTodoItems(), payload.now));
}

// 记录运行中计时器的最后心跳时间，避免浏览器重启后继续累加关闭时长。
async function touchTimer(payload) {
  return saveItems(touchRunningTodoTimer(await loadTodoItems(), payload.id, payload.now));
}

// 浏览器启动时把运行中的计时器恢复为暂停态。
async function pauseRunningTimersOnStartup() {
  const items = await loadTodoItems();
  const pausedItems = pauseRunningTodoTimers(items, new Date().toISOString(), { useLastTick: true });
  if (JSON.stringify(items) !== JSON.stringify(pausedItems)) await saveTodoItems(pausedItems);
}
...
async function completeTodoItem(items, item, receipt) {
  ...
  if (!receipt.appended) {
    if (!receipt.appendStarted || !Number.isInteger(receipt.matchingCountBefore)) {
      ...
      const result = await completedStore().appendCompletedRecord(completedRecordFromReceipt(receipt));
...
function isMutationMessage(type) {
  return new Set([
    ...
    MESSAGE_TYPES.TOGGLE_TODO_TIMER,
    MESSAGE_TYPES.TOGGLE_FOCUSED_TODO_TIMER,
    MESSAGE_TYPES.TOUCH_TODO_TIMER,
...
function completionReceiptFor(item, completedAt) {
  ...
  if ((existing?.appendStarted || existing?.appended) && Number.isFinite(Date.parse(existing.completedAt))) {
    const durationMs = normalizeCompletionDuration(existing.durationMs);
    return {
  ...
  const completedAtIso = Number.isNaN(time.getTime()) ? new Date().toISOString() : time.toISOString();
  const durationMs = normalizeCompletionDuration(getTodoTimerElapsedMs(item, completedAtIso));
  return {
...
// 从完成回执生成最终写入 JSON 的记录。
function completedRecordFromReceipt(receipt) {
  return {
    text: receipt.text,
    completedAt: receipt.completedAt,
    ...(normalizeCompletionDuration(receipt.durationMs) > 0 ? { durationMs: normalizeCompletionDuration(receipt.durationMs) } : {})
  };
}

// 标准化完成记录中的计时时长。
function normalizeCompletionDuration(value) {
