...
import { BALL_SIZE_DEFAULT, BALL_SIZE_MAX, BALL_SIZE_MIN } from "../shared/settings.js";
...
const elements = {
  ...
  ballSizePreview: document.getElementById("ballSizePreview"),
...
let settings = { ballSize: BALL_SIZE_DEFAULT, ballThemeColor: "#2563eb", colorPresets: [], recurringTasks: [] };
let chart = null;
...
function renderCompletedRecords() {
  ...
  for (const record of records) {
    ...
    const text = createCompletedTextElement(record.text);
    ...
    const durationText = formatCompletedDuration(record.durationMs);
    if (durationText) {
      const duration = document.createElement("span");
      duration.className = "completed-duration";
      duration.textContent = `用时 ${durationText}`;
...
// 根据完成记录文本创建只读文本或链接节点。
function createCompletedTextElement(value) {
  const parsedLink = parseCompletedRecordLink(value);
  if (!parsedLink) {
  ...
  link.className = "completed-text completed-link";
  link.href = parsedLink.url;
  link.target = "_blank";
  link.rel = "noopener noreferrer";
  link.title = parsedLink.label;
  link.textContent = parsedLink.label;
  return link;
}

// 解析完成记录里的 Markdown 链接或纯 URL。
function parseCompletedRecordLink(value) {
  const rawText = String(value || "").trim();
  const markdownMatch = rawText.match(/^\[([^\]\r\n]+)\]\((https?:\/\/[^\s<>"')]+)\)$/i);
  if (markdownMatch) {
    const label = markdownMatch[1].trim();
    const url = normalizeCompletedUrl(markdownMatch[2]);
    if (label && url) return { label, url };
  }
  const directUrl = normalizeCompletedUrl(rawText);
  if (directUrl) return { label: rawText, url: directUrl };
  return null;
}

// 校验完成记录链接，只允许普通网页协议。
function normalizeCompletedUrl(value) {
  try {
...
function renderBallThemeColor() {
  const color = normalizePickerColor(settings.ballThemeColor || "#2563eb");
  elements.ballThemeColor.value = color;
  elements.ballSizePreview.style.setProperty("--preview-ball-theme", color);
}

// 渲染悬浮球大小设置和实时预览。
function renderBallSize(value = settings.ballSize) {
  const ballSize = normalizeBallSize(value);
  elements.ballSize.value = String(ballSize);
  elements.ballSizeValue.textContent = `${ballSize}px`;
  elements.ballSizePreview.style.setProperty("--preview-ball-size", `${ballSize}px`);
}
...
// 保存悬浮球大小设置。
async function updateBallSize(value) {
  const ballSize = normalizeBallSize(value);
  const result = await sendMessage(MESSAGE_TYPES.UPDATE_SETTINGS, { ballSize });
  if (!result.ok) return;
  settings = result.settings || { ...settings, ballSize };
  renderBallSize();
}
...
// 将完成记录的毫秒用时格式化为短文本。
function formatCompletedDuration(durationMs) {
...
elements.ballSize.addEventListener("input", () => {
  renderBallSize(elements.ballSize.value);
});
elements.ballSize.addEventListener("change", () => {
  updateBallSize(elements.ballSize.value);
});
