...
const TASK_LABEL_LINE_HEIGHT = 13;
...
function renderTaskBlock(params, api) {
  ...
  const cellHeight = Math.max(18, cellSize[1] - 8);
  const blockGap = itemTotal > 2 ? 2 : TASK_BLOCK_GAP;
  const totalGap = blockGap * Math.max(0, itemTotal - 1);
  const blockHeight = Math.max(12, Math.min(30, (cellHeight - totalGap) / itemTotal));
  ...
  const rect = {
    ...
    y: stackTop + itemIndex * (blockHeight + blockGap)
  };
...
function formatTaskLabel(text, width, height) {
  ...
  for (const unit of units) {
    const unitWidth = getTextWidth(unit);
    if (line && lineWidth + unitWidth > width) {
      if (lines.length === maxLines - 1) return [...lines, truncateLine(line, width)].join("\n");
      lines.push(line);
      line = unit;
      lineWidth = unitWidth;
    } else {
  ...
  if (lines.length > maxLines) {
    return [...lines.slice(0, maxLines - 1), truncateLine(lines[maxLines - 1], width)].join("\n");
  }
  return lines.join("\n");
}
...
function truncateLine(text, width) {
  ...
  for (const unit of Array.from(text)) {
    const unitWidth = getTextWidth(unit);
    if (resultWidth + unitWidth + ellipsisWidth > width) break;
    result += unit;
...
function getTextWidth(text) {
  return Array.from(text).reduce((width, unit) => width + getTextUnitWidth(unit), 0);
}

function getTextUnitWidth(unit) {
  if (/\s/.test(unit)) return 3.5;
  if (/[\x00-\x7F]/.test(unit)) return 6.6;
  if (/[\u4E00-\u9FFF\u3400-\u4DBF\uF900-\uFAFF]/.test(unit)) return 12;
  return 10;
}
