...
test("completed file store appends duration when the completed record has timing", async () => {
  const writes = [];
  globalThis.indexedDB = createIndexedDbStub();
  globalThis.showSaveFilePicker = async () => createHandle("completed.json", "", writes);
  globalThis.chrome = createChromeStorage().chrome;

  const store = await import(`../src/shared/completed-file-store.js?test=${Date.now()}-duration-record`);
  await store.createCompletedJsonFile();
  await store.appendCompletedRecordToFile({
    text: "Timed task",
  ...
  assert.deepEqual(JSON.parse(writes.at(-1)).completed, [
    { text: "Timed task", completedAt: "2026-07-23T09:30:00.000Z", durationMs: 90000 }
  ]);
});
