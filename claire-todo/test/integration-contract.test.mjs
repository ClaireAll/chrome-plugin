...
test("completed records contain text, completedAt, and optional duration", () => {
  const data = appendCompletedRecord(createEmptyCompletedData(), {
    text: "Task A",
  ...
  assert.deepEqual(Object.keys(data.completed[0]).sort(), ["completedAt", "durationMs", "text"]);
});
