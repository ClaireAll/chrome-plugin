...
test("complete message appends elapsed timer duration when a timed todo is completed", async (t) => {
  const chromeStub = installChromeStub(t);
  const worker = await importWorker("complete-timed");
  ...
  const result = await complete(worker, "a", "2026-07-23T09:02:00.000Z");

  assert.equal(result.ok, true);
  assert.deepEqual(chromeStub.appended, {
    text: "Task A",
    completedAt: "2026-07-23T09:02:00.000Z",
    durationMs: 150000
  });
});

test("timer messages allow multiple running todos and keep the earliest focused", async (t) => {
  const chromeStub = installChromeStub(t);
  const worker = await importWorker("timer-toggle");
  chromeStub.values.todoUnfinishedItems = [
    {
      id: "a",
      text: "Task A",
  ...
  const result = await worker.handleMessage({
    ...
    payload: { id: "b", now: "2026-07-23T09:01:00.000Z" }
  });

  assert.equal(result.ok, true);
  assert.equal(result.items[0].timerState, "running");
  assert.equal(result.items[0].timerElapsedMs, 30000);
  assert.equal(result.items[0].timerFocused, true);
  assert.equal(result.items[1].timerState, "running");
  assert.equal(result.items[1].timerFocused, false);
  assert.equal(result.items[1].timerFirstStartedAt, "2026-07-23T09:01:00.000Z");
});

test("focused timer message pauses and resumes all active timers together", async (t) => {
  const chromeStub = installChromeStub(t);
  const worker = await importWorker("timer-toggle-all");
  chromeStub.values.todoUnfinishedItems = [
    {
      id: "a",
      text: "Task A",
    ...
    },
    {
      ...
      timerState: "running",
      timerElapsedMs: 0,
      timerStartedAt: "2026-07-23T09:01:00.000Z",
      timerLastTickAt: "2026-07-23T09:01:00.000Z",
      timerFirstStartedAt: "2026-07-23T09:01:00.000Z",
      timerFocused: false
    }
  ];
  ...
  const paused = await worker.handleMessage({
    ...
    payload: { now: "2026-07-23T09:05:00.000Z" }
  });
  const resumed = await worker.handleMessage({
    type: MESSAGE_TYPES.TOGGLE_FOCUSED_TODO_TIMER,
    payload: { now: "2026-07-23T09:06:00.000Z" }
  });

  assert.deepEqual(paused.items.map((item) => item.timerState), ["paused", "paused"]);
  assert.deepEqual(paused.items.map((item) => item.timerElapsedMs), [300000, 240000]);
  assert.deepEqual(resumed.items.map((item) => item.timerState), ["running", "running"]);
  assert.deepEqual(resumed.items.map((item) => item.timerFocused), [true, false]);
});

test("startup pauses a running timer at its last heartbeat", async (t) => {
  const chromeStub = installChromeStub(t);
  await importWorker("timer-startup");
  chromeStub.values.todoUnfinishedItems = [{
    id: "a",
    text: "Task A",
    color: "#fff",
    ...
    timerElapsedMs: 30000,
    timerStartedAt: "2026-07-23T09:00:00.000Z",
    timerLastTickAt: "2026-07-23T09:00:45.000Z",
    timerFocused: true
  }];
  ...
  assert.equal(chromeStub.values.todoUnfinishedItems[0].timerState, "paused");
  assert.equal(chromeStub.values.todoUnfinishedItems[0].timerElapsedMs, 75000);
  assert.equal(chromeStub.values.todoUnfinishedItems[0].timerStartedAt, "");
});
...
function createChromeStub(options = {}) {
  ...
  const runtime = {
    ...
    onStartup: { addListener(callback) { stub.startup = callback; } },
