...
test("settings sanitizes ball position and color presets", async () => {
  ...
  assert.equal(settings.ballSize, 88);
  assert.deepEqual(settings.colorPresets, ["#ffffff", "#dcfce7"]);
  assert.equal(settings.defaultColor, "#dcfce7");
});

test("settings keeps the default ball size when input is invalid", async () => {
  const { sanitizeSettings } = await import(`../src/shared/settings.js?test=${Date.now()}-ball-size-default`);

  assert.equal(sanitizeSettings({ ballSize: "large" }).ballSize, 44);
  assert.equal(sanitizeSettings({ ballSize: 20 }).ballSize, 36);
});
